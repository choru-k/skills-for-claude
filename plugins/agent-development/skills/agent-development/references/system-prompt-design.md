...
Complete guide to writing effective subagent system prompts that enable autonomous, high-quality operation.

## Core Structure

Every subagent system prompt should follow this proven structure:
...
If your subagent uses the `memory` frontmatter field (e.g., `memory: user`), include instructions in the system prompt about what to remember and when to check memory.

**Memory scopes:** `user` (`~/.claude/agent-memory/<name>/`), `project` (`.claude/agent-memory/<name>/`), `local` (`.claude/agent-memory-local/<name>/`). The system automatically includes the first 200 lines of `MEMORY.md` and enables Read/Write/Edit tools.

```markdown
**Memory:**
Update your agent memory as you discover patterns, conventions, and
architectural decisions. Check memory at the start of each run for
previously learned patterns.
```

This ensures the subagent actively reads from and writes to its persistent memory, making it smarter over time.

## Pattern 1: Analysis Subagents

For subagents that analyze code, PRs, or documentation:
...
## Pattern 2: Generation Subagents

For subagents that create code, tests, or documentation:
...
## Pattern 3: Validation Subagents

For subagents that validate, check, or verify:
...
## Pattern 4: Orchestration Subagents

For subagents that coordinate multiple tools or steps:
...
Can the subagent handle these based on system prompt alone?
...
Use the patterns above as templates, customize for your domain, and iterate based on subagent performance.
