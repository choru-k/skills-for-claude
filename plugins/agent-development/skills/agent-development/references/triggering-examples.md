# Subagent Triggering Examples: Best Practices

Complete guide to writing effective `<example>` blocks in subagent descriptions for reliable triggering.
...
**Purpose:** Show how Claude invokes the subagent via the Task tool

**Standard pattern:**
```
assistant: "I'll use the [agent-name] agent to [what it will do]."
assistant: Uses the Task tool to launch the [agent-name] subagent
```
...
- Show Claude using the Task tool to invoke the subagent

❌ **DON'T:**
- Use generic, vague examples
- Omit context or commentary
- Show only one type of triggering
- Skip the subagent invocation step
- Make examples too similar
- Forget to explain why subagent triggers

## Conclusion

Well-crafted examples are crucial for reliable subagent triggering. Invest time in creating diverse, specific examples that clearly demonstrate when and why the subagent should be used.
